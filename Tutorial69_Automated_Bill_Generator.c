...
int main()
{
    FILE * ptr = NULL;
    FILE * ptr2 = NULL;
    ptr = fopen("Tutorial69_Bill.txt", "r");
    ptr2 = fopen("Tutorial69_GenBill.txt", "w");
    char str [200];
    fgets(str, 200, ptr);
    ...
    char * newStr;
    newStr = replaceWord(str, "{{Item}}", "MacBook");
    newStr = replaceWord(newStr, "{{Company}}", "Apple");
    newStr = replaceWord(newStr, "{{Name}}", "Avnish");
